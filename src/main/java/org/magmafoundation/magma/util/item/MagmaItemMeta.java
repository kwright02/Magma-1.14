...
import org.magmafoundation.magma.util.DeprecatedCustomTagContainer;
...
public class MagmaItemMeta implements ItemMeta, Damageable, Repairable, BlockDataMeta {
    ...
    public static class SerializableMeta implements ConfigurationSerializable {
        ...
        static final ImmutableMap<Class<? extends MagmaItemMeta>, String> classMap;
        static final ImmutableMap<String, Constructor<? extends MagmaItemMeta>> constructorMap;

        static {
            classMap = ImmutableMap.<Class<? extends MagmaItemMeta>, String>builder()
//                    .put(CraftMetaArmorStand.class, "ARMOR_STAND") TODO 
...
//                    .put(CraftMetaSuspiciousStew.class, "SUSPICIOUS_STEW")
                    .put(MagmaItemMeta.class, "UNSPECIFIC")
                    .build();

            final ImmutableMap.Builder<String, Constructor<? extends MagmaItemMeta>> classConstructorBuilder = ImmutableMap.builder();
            for (Map.Entry<Class<? extends MagmaItemMeta>, String> mapping : classMap.entrySet()) {
                try {
        ...
        public static ItemMeta deserialize(Map<String, Object> map) throws Throwable {
            ...
            Constructor<? extends MagmaItemMeta> constructor = constructorMap.get(type);
    ...
    private static final MagmaPersistentDataTypeRegistry DATA_TYPE_REGISTRY = new MagmaPersistentDataTypeRegistry();

    private CompoundNBT internalTag;
    private final Map<String, INBT> unhandledTags = new HashMap<String, INBT>();
    private final MagmaPersistentDataContainer persistentDataContainer = new MagmaPersistentDataContainer(DATA_TYPE_REGISTRY);

    private int version = MagmaUnsafeValues.getInstance().getDataVersion(); // Internal use only

    MagmaItemMeta(MagmaItemMeta meta) {
        if (meta == null) {
            return;
        }
    ...
    MagmaItemMeta(CompoundNBT tag) {
        if (tag.keySet().contains(DISPLAY.NBT)) {
            CompoundNBT display = tag.getCompound(DISPLAY.NBT);

            if (display.keySet().contains(NAME.NBT)) {
                try {
            ...
            if (display.keySet().contains(LOCNAME.NBT)) {
                try {
            ...
            if (display.keySet().contains(LORE.NBT)) {
                ListNBT list = display.getList(LORE.NBT, MagmaUnsafeValues.NBT.TAG_STRING);
        ...
        if (tag.keySet().contains(CUSTOM_MODEL_DATA.NBT)) {
            customModelData = tag.getInt(CUSTOM_MODEL_DATA.NBT);
        }
        if (tag.keySet().contains(BLOCK_DATA.NBT)) {
        ...
        if (tag.keySet().contains(REPAIR.NBT)) {
            repairCost = tag.getInt(REPAIR.NBT);
        }

        if (tag.keySet().contains(HIDEFLAGS.NBT)) {
            hideFlag = tag.getInt(HIDEFLAGS.NBT);
        }
        if (tag.keySet().contains(UNBREAKABLE.NBT)) {
            unbreakable = tag.getBoolean(UNBREAKABLE.NBT);
        }
        if (tag.keySet().contains(DAMAGE.NBT)) {
            damage = tag.getInt(DAMAGE.NBT);
        }
        if (tag.keySet().contains(BUKKIT_CUSTOM_TAG.NBT)) {
    ...
    static Map<Enchantment, Integer> buildEnchantments(CompoundNBT tag, ItemMetaKey key) {
        if (!tag.keySet().contains(key.NBT)) {
            return null;
        }

        ListNBT ench = tag.getList(key.NBT, MagmaUnsafeValues.NBT.TAG_COMPOUND);
        ...
        for (int i = 0; i < ench.size(); i++) {
            ...
            Enchantment enchant = Enchantment.getByKey(MagmaNamespacedKey.fromStringOrNull(id));
            if (enchant != null) {
    ...
    static Multimap<Attribute, AttributeModifier> buildModifiers(CompoundNBT tag, ItemMetaKey key) {
        ...
        ListNBT mods = tag.getList(key.NBT, MagmaUnsafeValues.NBT.TAG_COMPOUND);
        int size = mods.size();
        ...
        for (int i = 0; i < size; i++) {
            ...
            AttributeModifier attribMod = MagmaAttributeInstance.convertToBukkit(nmsModifier);
            ...
            Attribute attribute = MagmaAttributeMap.fromMinecraft(attributeName);
            if (attribute == null) {
                continue;
            }

            if (entry.keySet().contains(ATTRIBUTES_SLOT.NBT)) {
                ...
                try {
                    slot = MagmaEquipmentSlot.getSlot(EquipmentSlotType.valueOf(slotName.toLowerCase(Locale.ROOT)));
    ...
    MagmaItemMeta(Map<String, Object> map) {
        ...
        if (blockData != null) {
            this.blockData = (CompoundNBT) MagmaNBTConfigSerializer.deserialize(blockData);
        }
        ...
        if (internal != null) {
            ...
            try {
                ...
                Logger.getLogger(MagmaItemMeta.class.getName()).log(Level.SEVERE, null, ex);
            }
        }

        Map nbtMap = SerializableMeta.getObject(Map.class, map, BUKKIT_CUSTOM_TAG.BUKKIT, true);
        if (nbtMap != null) {
            this.persistentDataContainer.putAll((CompoundNBT) MagmaNBTConfigSerializer.deserialize(nbtMap));
        }
    }

    void deserializeInternal(CompoundNBT tag, Object context) {
        // SPIGOT-4576: Need to migrate from internal to proper data
        if (tag.keySet().contains(ATTRIBUTES.NBT)) {
            this.attributeModifiers = buildModifiers(tag, ATTRIBUTES);
        }
    }
    ...
    void applyToItem(CompoundNBT itemTag) {
        if (hasDisplayName()) {
            setDisplayTag(itemTag, NAME.NBT, new StringNBT(MagmaChatMessage.toJSON(displayName)));
        }
        if (hasLocalizedName()){
            setDisplayTag(itemTag, LOCNAME.NBT, new StringNBT(MagmaChatMessage.toJSON(locName)));
        }
        ...
        if (!persistentDataContainer.isEmpty()) {
            ...
            for (Map.Entry<String, INBT> INBTEntry : rawPublicMap.entrySet()) {
                bukkitCustomCompound.put(INBTEntry.getKey(), INBTEntry.getValue());
            }
    ...
    ListNBT createStringList(List<ITextComponent> list) {
        ...
        for (ITextComponent value : list) {
            ...
            tagList.add(new StringNBT(version <= 0 || version >= 1803 ? MagmaChatMessage.toJSON(value) : MagmaChatMessage.fromComponent(value, TextFormatting.DARK_PURPLE))); // SPIGOT-4935
        }
    ...
    static void applyModifiers(Multimap<Attribute, AttributeModifier> modifiers, CompoundNBT tag, ItemMetaKey key) {
        ...
        for (Map.Entry<Attribute, AttributeModifier> entry : modifiers.entries()) {
            ...
            net.minecraft.entity.ai.attributes.AttributeModifier nmsModifier = MagmaAttributeInstance.convertToNMS(entry.getValue());
            CompoundNBT sub = SharedMonsterAttributes.writeAttributeModifier(nmsModifier);
            if (sub.isEmpty()) {
                continue;
            }

            String name = MagmaAttributeMap.toMinecraft(entry.getKey());
            ...
            if (entry.getValue().getSlot() != null) {
                EquipmentSlotType slot = MagmaEquipmentSlot.getNMS(entry.getValue().getSlot());
                if (slot != null) {
    ...
    public String getDisplayName() {
        return MagmaChatMessage.fromComponent(displayName, TextFormatting.WHITE);
    }

    @Override
    public final void setDisplayName(String name) {
        this.displayName = MagmaChatMessage.wrapOrNull(name);
    }
    ...
    public String getLocalizedName() {
        return MagmaChatMessage.fromComponent(locName, TextFormatting.WHITE);
    }

    @Override
    public void setLocalizedName(String name) {
        this.locName = MagmaChatMessage.wrapOrNull(name);
    }
    ...
    public List<String> getLore() {
        return this.lore == null ? null : new ArrayList<>(this.lore.stream().map((line) -> MagmaChatMessage.fromComponent(line, TextFormatting.DARK_PURPLE)).collect(Collectors.toList()));
    }
...
//        return CraftBlockData.fromData(MagmaUnsafeValues.getInstance().getBlock(material).getBlockData(), blockData);
//    }
//
//    @Override
...
//    }
    ...
    public final boolean equals(Object object) {
        ...
        if (!(object instanceof MagmaItemMeta)) {
            return false;
        }
        return MagmaItemFactory.instance().equals(this, (ItemMeta) object);
    }
    ...
    boolean equalsCommon(MagmaItemMeta that) {
    ...
    boolean notUncommon(MagmaItemMeta meta) {
        return true;
    }
    ...
    @Override
    public MagmaItemMeta clone() {
        try {
            MagmaItemMeta clone = (MagmaItemMeta) super.clone();
            if (this.lore != null) {
    ...
    ImmutableMap.Builder<String, Object> serialize(ImmutableMap.Builder<String, Object> builder) {
        if (hasDisplayName()) {
            builder.put(NAME.BUKKIT, MagmaChatMessage.fromComponent(displayName));
        }
        if (hasLocalizedName()) {
            builder.put(LOCNAME.BUKKIT, MagmaChatMessage.fromComponent(locName));
        }

        if (hasLore()) {
            builder.put(LORE.BUKKIT, ImmutableList.copyOf(Lists.transform(lore, MagmaChatMessage::fromComponent)));
        }
        ...
        if (hasBlockData()) {
            builder.put(BLOCK_DATA.BUKKIT, MagmaNBTConfigSerializer.serialize(blockData));
        }
        ...
        if (!internalTags.isEmpty()) {
            ...
            try {
                ...
                builder.put("internal", Base64.getDecoder().decode(buf.toByteArray()));
            } catch (IOException ex) {
                Logger.getLogger(MagmaItemMeta.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    static void safelyAdd(Iterable<?> addFrom, Collection<ITextComponent> addTo, int maxItemLength) {
        ...
        for (Object object : addFrom) {
            if (!(object instanceof String)) {
                ...
                addTo.add(new TranslationTextComponent(""));
            } else {
                ...
                addTo.add(MagmaChatMessage.wrapOrEmpty(page));
            }
        }
    }
        ...
        synchronized (HANDLED_TAGS) {
            if (HANDLED_TAGS.isEmpty()) {
                HANDLED_TAGS.addAll(Arrays.asList(
                        ...
                        ATTRIBUTES_SLOT.NBT
//                        CraftMetaMap.MAP_SCALING.NBT, TODO to Implement
